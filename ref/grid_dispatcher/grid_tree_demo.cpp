...
struct sample_tree : grid_tree_t<box> {
	using base = grid_tree_t<box>;
	sample_tree() noexcept : grid_tree_t<box>(box(float3(0, 0, 0), float3(0, 0, 0)), 0) {}
	sample_tree(const box& b, uint8_t k) noexcept : grid_tree_t<box>(b, k) {}
};
